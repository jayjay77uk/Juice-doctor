...
**Context.** The platform serves six distinct actors — an unauthenticated visitor, a registered client, a practitioner, operational staff, an org administrator, and the platform owner. Real service products tend to sprawl into a matrix of overlapping custom roles that nobody can reason about. We needed a role model a non-technical operator could hold in their head, that still expressed genuine privilege boundaries.
...
**Context.** The product needs multiple assistants (a member-facing assistant, an intake triage bot, a practitioner co-pilot) and the client will want more over time — with different prompts, personalities, tools, models, and safety rules — *without* an engineer editing and redeploying code for each. No inference is built in Phase 2; but the *shape* that later carries inference must not hard-code the agents.

**Decision.** Represent each agent as **rows, not functions.** The schema (migration `0009`) models `ai_agents` + `ai_agent_versions` + `ai_agent_tools` + `ai_agent_knowledge_sources`, alongside `ai_model_providers` / `ai_models` (future model selection), `ai_tools`, and `ai_configurations`. Each agent carries its `id`, `name`, `description`, `role`, `personality`, `system_prompt`, `temperature`, token limits, `default model`, `memory_config`, `safety_rules`, `visibility` (private/organisation/public), `status` (draft/active/disabled/archived), `version`, and `owner`. The typed shape lives in `src/types/ai.ts` and `src/services/agents.ts` reads the rows. **No inference ran in Phase 2.** (Update: inference is now live against Anthropic Claude, and the original three seed agents were replaced by the receptionist in `src/config/ai-agents.ts` plus the eight HERNE specialists seeded by `services/herne/seed.ts` — validating the decision: the roster changed with no code-path change.)
...
**Decision.** Place the edge middleware in **`src/proxy.ts`**, exporting the default `proxy` handler and the `config.matcher`. It (1) attaches `securityHeaders(...)` to every matched response, (2) blocks cross-origin `POST/PUT/PATCH/DELETE` via `isSameOrigin` (the CSRF Origin check), and (3) guards protected route prefixes. Route protection was originally production-shaped but bypassed while there was no real auth; today it is **active**: the block refreshes the Supabase session and redirects unauthenticated users to `/login` whenever Supabase is configured, degrading to open only for keyless local development.

**Rationale.** Adopting the framework's new convention is not optional — `proxy.ts` is the name Next 16 actually loads, and a stale `middleware.ts` would run *nothing*. Keeping all three edge concerns in one file makes the request-entry security posture reviewable at a glance. Shipping the redirect logic in place while it was still bypassed meant activating real protection later was a wiring change, not new middleware — which is exactly how it played out.
...
**Decision.** Ship a **strict, conservative CSP baseline** (`src/lib/security/headers.ts`) and relax it **only in development, only for `script-src`.** `buildContentSecurityPolicy({ dev })` emits `'self' 'unsafe-inline' 'unsafe-eval'` for scripts *in dev only*; the builder supports `'self' 'nonce-…' 'strict-dynamic'` for production, falling back to `'self' 'unsafe-inline'` where no nonce is supplied. (Current state: `proxy.ts` does not yet mint a nonce, so production ships the fallback — wiring the nonce is a known hardening step.) The rest of the policy stays locked in every environment: `default-src 'self'`, `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`, `upgrade-insecure-requests`, plus a restrictive `Permissions-Policy` (only `camera=(self)` for the Selfie Scan). New sources (Supabase, Resend) are added **explicitly, with a comment, never by loosening globally.**
...
**Realised in:** `src/lib/security/headers.ts` (`buildContentSecurityPolicy`, `securityHeaders`), `src/proxy.ts` (`dev` wiring; nonce path not yet wired), `src/lib/security/csrf.ts`.
