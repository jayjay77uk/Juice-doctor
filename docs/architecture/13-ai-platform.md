...
| Configurable thing | Where it lives (config seed → live table) | Managed at | Editable by (permission) |
| --- | --- | --- | --- |
...
The seeded roster demonstrates the range. Originally three generic seed agents lived in [`ai-agents.ts`](../../src/config/ai-agents.ts) (Assistant AI, Intake & Triage, Practitioner Copilot); those have been retired. Today `ai-agents.ts` seeds the **Receptionist AI**, and the eight client-approved **HERNE specialists** are seeded by `services/herne/seed.ts` — all as rows in `ai_agents`, edited through the same editor.
...
Prompts are **agent‑scoped when `agent_id` is set, org‑global when it is null** — so a shared safety directive can be authored once and reused. The seed pipeline gives each agent a `system` prompt (the HERNE seeder writes real `ai_prompts` rows), and the **published version is exactly what the live runtime reads at inference time**.
...
The agent's `systemPrompt` field and the `system`‑kind prompt in Prompt Management describe the *same* behaviour at two altitudes: the agent editor's Behaviour tab is the quick‑edit convenience, while `/admin/ai/prompts/[id]` is the governed, versioned source of truth (its hint text says as much). The live runtime resolves an agent's prompt by reading the current **published** version of its `system` prompt at inference time, not a frozen string.
...
flowchart LR
  ...
  Q["Test query"] --> R
  R["playground.run()<br/>(LIVE inference)"] --> O["Model response"]
  R --> C["Retrieved chunks + scores"]
  R --> M["tokens in/out · latency · model"]
  R --> L["ai_run_logs (is_playground = true)"]
```

`playground.run()` is **real**: it retrieves the agent's assigned knowledge via ranked full-text search and reasons over it with live Anthropic inference, returning a `PlaygroundResult` with the actual answer, the genuinely retrieved `RetrievedChunk[]`, real `tokensInput`/`tokensOutput` and measured `latencyMs` (and it is honest when the AI is unavailable). `runPlaygroundAction` is the one Server Action that **returns a value** to the client rather than redirecting, because the console renders the result inline.
...
Real inference has landed exactly this way: `run()` became an isolated model call still stamped `is_playground = true`. The isolation contract did not move.

---

## 6. Safety management

Guardrails are modelled as configurable data. Two tables hold this: [`ai_safety_policies`](../../db/migrations/0014_ai_platform_management.sql#L64) (the reusable policy) and [`ai_agent_safety_policies`](../../db/migrations/0014_ai_platform_management.sql#L88) (a many‑to‑many binding of policies to agents). Served by [`safety.ts`](../../src/services/safety.ts); managed at `/admin/ai/safety` (admin‑only in RLS). **Current enforcement caveat:** the policies are stored as real, idempotently seeded rows, but the *runtime* safety checks that actually guard live inference (the pre-inference emergency/self-harm block with real escalation, and post-inference stripping of fabricated citations) are enforced in code today — wiring the stored policies into inference is a follow-up, as noted in the admin UI.

| Configurable control | Field | Seed example |
| --- | --- | --- |
...
Two seed policies show the split: a strict **Default policy** (safe by default, escalates on self‑harm/acute cases) and a looser, staff‑only **Practitioner‑copilot policy**. Because policies are separate rows bound to agents, one boundary change (e.g. adding a restricted topic) applies to every agent sharing that policy — no per‑agent editing, no redeploy. `safety.save()` handles both create (id `null`) and update as a real Supabase write.
...
Per‑agent memory configuration is stored on the agent itself as `memoryConfig` (mirrors `ai_agents.memory_config` jsonb): four booleans — `useUserMemory`, `useConversationMemory`, `useOrganisationMemory`, `useGlobalMemory` — plus `maxItems` capping how much is injected into context. This is edited on the agent editor's Memory tab; the cross-agent Memory Centre page (`/admin/ai/memory`) is illustrative today ("coming soon"). The `ai_memory` table itself is real and written by the live runtime, and RLS on `ai_memory` (0011) enforces the same isolation the selector expresses in code.

---

## 8. Analytics foundation

Observability was designed as an **append‑only event stream plus a pre‑aggregated daily rollup** so dashboards stay fast and history immutable. Today the live figures are computed directly from real operational rows — chiefly `ai_run_logs`, which records tokens, cost, latency and trace for every inference call — with the 0014 event/rollup tables available as the scaling path ([`analytics.ts`](../../src/services/analytics.ts)):
...
flowchart LR
  ...
  SVC --> DB["Supabase tables<br/>0009 · 0011 · 0012 · 0013 · 0014<br/>+ RLS"]
```

| Domain | Phase‑3 store (since replaced) | Live table(s) today | Interface changed? |
| --- | --- | --- | --- |
...
| `/admin/knowledge` (`/[id]`) · `/admin/users` | User dashboard is served by [`member.ts`](../../src/services/member.ts) reading the member's real rows; AI chat is live (streaming specialist conversations). |
