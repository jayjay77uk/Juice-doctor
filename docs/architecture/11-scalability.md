...
> **Why pay up front for tenancy we don't use yet?** Because the cost curve is brutally asymmetric. Adding `organisation_id` to an empty table in migration `0002` is one line. Retrofitting it into 55 populated, RLS-protected, foreign-keyed tables in production is a multi-week migration with a data-integrity risk on every row. The platform currently has exactly one organisation and the column is effectively constant — but it is *there*, indexed, and enforced by RLS, so going multi-tenant is a `INSERT INTO organisations` away.
...
The AI framework ([`db/migrations/0009_ai_agents.sql`](../../db/migrations/0009_ai_agents.sql), [`src/types/ai.ts`](../../src/types/ai.ts)) treats agents, tools, models, and providers as **rows**, not classes. An agent is a `system_prompt`, a `temperature`, a `memory_config`, a set of `safety_rules`, a visibility, a status, and a version — all columns. The seeded roster (the receptionist in [`src/config/ai-agents.ts`](../../src/config/ai-agents.ts) and the eight HERNE specialists in `services/herne/seed.ts`) is the *proof*: declarative definitions, not code paths. The tenth or hundredth agent is another row. The same pattern governs model **providers** and **models** — the platform is provider-agnostic by construction.
...
|---|---|---|
| **Multiple organisations** | `organisations` table (`0002`); `organisation_id` on every tenant table; `organisation_memberships` join (a user may belong to many orgs); RLS scoped by `app.current_org_id()`; super-admin platform reads via `app.is_super_admin()`. The platform runs with exactly one org row. | Org-provisioning/onboarding flow; an org-switcher in the UI; billing per org; resolving `current_org_id` from the active membership at request time. **No schema change** — isolation already exists. |
...
| **Multiple AI agents** | Agents-as-data: `ai_agents` + `ai_agent_versions` + `ai_agent_tools` + `ai_agent_knowledge_sources` (`0009`); `AiAgentDefinition` in [`src/types/ai.ts`](../../src/types/ai.ts); the Receptionist AI seeded from [`src/config/ai-agents.ts`](../../src/config/ai-agents.ts) and the eight HERNE specialists from `src/services/herne/seed.ts`; per-agent visibility (`private`/`organisation`/`public`), `status`, `version`, `owner`, `memory_config`, `safety_rules`. | **Both now built:** the admin authoring UI exists, and the live runtime assembles each agent's behaviour from data and calls Anthropic Claude. **Adding an agent is an `INSERT`**, not a code change — this is the framework's headline claim, and it now runs for real. |
...
flowchart LR
  ...
  subgraph Remaining["Remaining per capability (the cheap part)"]
    E["Provider wiring<br/>(Supabase + Claude done;<br/>device/STT connectors remain)"]
...
- **Another agent needs zero deploy.** The seeded roster (the receptionist from [`src/config/ai-agents.ts`](../../src/config/ai-agents.ts) plus the eight HERNE specialists from `services/herne/seed.ts`) is data in `ai_agents`. The admin UI writing another row is the same operation. No branch, no build, no release.
