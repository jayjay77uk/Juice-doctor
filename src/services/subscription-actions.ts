...
async function queueStateMail(sub: CustomerSubscription): Promise<void> {
  ...
  try { await sendTemplateMail({ to: sub.customerEmail, template: 'subscription.state_changed', params: { planName: sub.planName, state: sub.state }, dedupeKey: `sub-state:${sub.id}:${sub.state}:${sub.updatedAt}` }); } catch {}
}
...
async function assertOwnLiveSubscription(id: string, userId: string): Promise<{ ok: true } | Res> {
  ...
  if (!MEMBER_LIVE_STATES.includes(sub.data.state)) return { ok: false, error: 'This subscription is not active — please contact the team to reactivate it.' };
  return { ok: true };
}

function revalidateAll(): void {
  revalidatePath('/admin/subscriptions'); revalidatePath('/dashboard/subscriptions'); revalidatePath('/dashboard/specialists'); revalidatePath('/admin');
}
...
export async function createPlanAction(_prev: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  const scope = String(formData.get('scope') ?? 'single') as SubscriptionScope; const specialistSlugs = toList(formData.get('specialistSlugs'));
  ...
  if (!result.ok) return { status: 'error', message: result.error.message }; revalidateAll(); return { status: 'success', message: 'Plan created.' };
}
...
/** Immediate free/demo activation for signed-in members. Paid checkout can replace this later. */
export async function customerSubscribeAction(planId: string): Promise<Res> {
  let session;
  try { session = await assertSession(); } catch { return { ok: false, error: 'Please sign in.' }; }
  ...
  const existing = await subscriptionsService.byMember(session.user.id);
  if (existing.ok && existing.data.some((s) => MEMBER_LIVE_STATES.includes(s.state) && s.planId === planId)) return { ok: true };
  const metadata = (session.user.user_metadata ?? {}) as Record<string, unknown>;
  const customerName = String(metadata.full_name ?? metadata.name ?? session.user.email?.split('@')[0] ?? 'Member');
  const customerEmail = session.user.email ?? '';
  const r = await subscriptionsService.create({ memberId: session.user.id, customerName, customerEmail, planId });
