 ...
 */
export async function deliverPendingMail(): Promise<{ available: boolean; pending: number; sent: number; failed: number }> {
  ...
  for (const row of rows) {
    ...
    await mailRepo.markAttempt(row.id, {
      status: result.sent ? 'sent' : result.retryable && attempts < MAX_ATTEMPTS ? 'queued' : 'failed',
      providerMessageId: result.sent ? result.providerMessageId : null,
      error: result.sent ? null : result.error,
      attempts,
    });
...
export async function mailStatus(): Promise<{
  ...
  return { configured: isMailConfigured(), addresses: businessAddresses(), outboxAvailable: available, counts };
}
