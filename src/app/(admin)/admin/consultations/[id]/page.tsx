    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="flex flex-col gap-8">
              ...
              {events.map((item) => (
                <li key={item.id} className="relative">
                  ...
                  <p className="mt-0.5 text-sm text-muted-foreground">
                    {item.actorName} · {fmt(item.createdAt)}
                  </p>
                </li>
              ))}
            </ol>
          ...
          <Panel title="Case notes" description="Notes are append-only and preserved in the timeline above.">
            <form action={addConsultationNoteAction} className="flex flex-col gap-2">
              ...
              <textarea
                ...
                minLength={2}
                maxLength={1000}
                placeholder="Add a note — it appends a new, immutable entry to the timeline."
        ...
        <div className="flex flex-col gap-8">
          <Panel title="Review">
            {CASE.aiReview && <p className="text-sm text-foreground">{CASE.aiReview}</p>}
            {!CASE.aiReview && <p className="text-sm text-muted-foreground">No AI review has been drafted for this case.</p>}
            ...
            {reviewable ? (
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <form action={reviewConsultationAction}>
                  ...
                  <Button type="submit" size="sm" intent="outline">
                    Request changes
                  </Button>
                </form>
              </div>
            ) : (
              <p className="mt-4 text-sm text-muted-foreground">This case is {CASE.status.replaceAll('_', ' ')} — no review action needed.</p>
            )}
            <p className="mt-3 text-xs text-muted-foreground">
              Approving completes the case; requesting changes returns it to awaiting review. Both are recorded in the timeline and audit log.
            </p>
          </Panel>
